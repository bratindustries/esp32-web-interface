...
**A modern web interface for [OpenInverter](https://openinverter.org) systems — for the LILYGO T-Embed over CAN bus.**
...
### 1. Get an image
Pre-built **flash-at-`0x0` combined images** for all supported boards:
...
esptool.py --chip esp32s3 write_flash 0x0 esp32_tembed_v4.1-0x000.bin  # original LILYGO T-Embed
```
...
- **CI** — every push builds all three targets and publishes combined images ([workflow](.github/workflows/build.yml)); pushing a `v*` tag creates a release with version-stamped binaries

Build targets: `esp32_wemos` / `esp32_wemos_debug` (classic ESP32 / Wemos), `esp32_t2can` / `esp32_t2can_debug` (LILYGO T-2Can, ESP32-S3), and `esp32_tembed` (original LILYGO T-Embed with external CAN transceiver).
